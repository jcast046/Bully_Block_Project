...
describe('Login Component', () => {
    ...
    test('successful login submits form and navigates to dashboard', async () => {
        ...
        // Verify that the token was saved in local storage
        await waitFor(() => {
            expect(localStorage.getItem('token')).toBe('fake-jwt-token');
        });
        // Verify that the login function from AuthContext was called with the user data
        expect(loginMock).toHaveBeenCalledWith(fakeResponse.data.user);
